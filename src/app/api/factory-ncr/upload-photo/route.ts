...
// POST /api/factory-ncr/upload-photo  (multipart/form-data)
...
export async function POST(req: NextRequest) {
  ...
  const objectKey = `factory-ncr/${ref}/${key}-${Date.now()}.${ext}`;
  ...
  try {
    ...
    console.error('[factory-ncr upload-photo] R2 PUT failed', err);
