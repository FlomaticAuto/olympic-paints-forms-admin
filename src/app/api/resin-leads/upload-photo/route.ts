...
// POST /api/resin-leads/upload-photo  (multipart/form-data)
// Fields: file (File), key (string), ref (string — the lead_ref)
export async function POST(req: NextRequest) {
  ...
  const ref  = (fd.get('ref') as string | null)?.trim().toUpperCase() ?? 'UNREF';

  if (!(file instanceof Blob) || !key) {
    return NextResponse.json({ error: 'Missing file or key' }, { status: 400, headers: corsHeaders(origin) });
  }

  const ext       = (file instanceof File ? file.name.split('.').pop() : '') || 'jpg';
  const objectKey = `resin-visits/${ref}/${key}-${Date.now()}.${ext}`;
  ...
  try {
    await r2Put(endpoint, mimeType, bodyBuf, R2_ACCESS_KEY, R2_SECRET_KEY);
  } catch (err) {
    console.error('[resin-leads/upload-photo] R2 PUT failed', err);
